...
TEST(functional1, load) {
	...
	EXPECT_TRUE(duration_cast<milliseconds>( end - begin ).count() < 150);
}
...
TEST(functional1, sequential_verify) {
	...
	for (size_t iter = 0; iter < 1000; ++iter) {
		...
		a.add(r.back());
		p.push_back(a.create_witness(iter));
	}
	for (size_t iter = 0; iter < 1000; ++iter) {
		EXPECT_TRUE(a.verify_witness(r[iter], iter, iter + 1, p[iter]));
	}
}
...
TEST(functional1, unsequential_verify) {
	...
	for (size_t iter = 0; iter < 1000; ++iter) {
		r.push_back(std::to_string(TestUtil::rnd()));
		l.push_back(TestUtil::rnd() % (1 + iter));
		a.add(r.back());
		p.push_back(a.create_witness(l.back(), iter + 1));
	}
	for (size_t iter = 0; iter < 1000; ++iter) {
		EXPECT_TRUE(a.verify_witness(r[iter], l[iter], iter + 1, p[iter]));
	}
}
